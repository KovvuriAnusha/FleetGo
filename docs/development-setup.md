...
The API will not start without two things configured. Both use `dotnet user-secrets`, so
neither ever touches a file that gets committed:
...
Leaving either unset is a deliberate fail-fast: `Jwt:SigningKey` is checked at startup
(`Program.cs` validates it via `ValidateOnStart()`) with an error message pointing back
here, and the database connection string is checked the first time anything touches
the database (a request, or `/health`) rather than silently falling back to something
insecure or wrong.

`Jwt:Issuer`, `Jwt:Audience`, `Jwt:AccessTokenLifetimeMinutes` and `RefreshToken:LifetimeDays`
all have non-secret defaults committed in `appsettings.json` - override them the same way
...
> **Note on this repository's own migration.** The initial `Users`/`Drivers`/`RefreshTokens`
...
> this branch, after setting up the two secrets above.
